...
export default function HcLogin() {
    ...
    const Fasubmit = async () => {
        if (!email || !password) {
            ...
            try {
                ...
                setTimeout(() => {
                    navigate(`/HcApplication?data=${encodeURIComponent(JSON.stringify(responseData))}`);
                }, 3000)
            } catch (error) {
                toast.dismiss(loadingNotification);
    ...
    const AppForm = async () => {
        try {
            const responseData = await axios.post("http://localhost:3000/hostel_coordinator/login", {
